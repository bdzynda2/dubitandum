...
__Definition__: $\mu^*:(T \cup B) \rightarrow (T \cup B)$ is _I-Optimal Stable match) if 

1. $\mu^*$ B stable
2. $\mu^* \ge_I \mu$ for all stable $\mu$


$\mu^* >_I \mu$ if $\mu^* \ge_I \mu$ and for some $i \in I: \mu^*(i) \succsim_i \mu(i)$




__Proposition__: Suppose preferences are strict. Then for each $I \in \{T,B\}$: $\mu^*_{ID}:(T \cup B) \rightarrow (T \cup B)$ is an I-optimal stable match. 
...
There is stable matchiing $\mu:(T \cup B) \rightarrow (T \cup B)$ with $\mu(t^*) = b^*$. 
