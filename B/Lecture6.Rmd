...
__Proof__: Suppos $\mu >_T \mu':$ to show:

(1) for each $b \in B$, $\mu'(b) \succsim_b \mu(b)$
(2) for some $b \in B$, $\mu'(b) \succ_b \mu(b)$


To show (1): Suppose not, i.e. there is some $b \in B$ such that $\mu(b) \succ_b \mu'(b)$
...
+ Hence, either (a) $\lambda(t^*) = \mu(t^*)$ and $\lambda_T(b^*) = \mu'(b^*)$ or (b) $\lambda(t^*) = \mu'(t^*)$ and $\lambda_T(b^*) = \mu(b^*)$
